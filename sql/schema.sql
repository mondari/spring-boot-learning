...
/*!
用户管理sys_user:保存密码、微信OPEN_ID
角色管理sys_role:用户和角色是多对多，角色和菜单是多对多。目前有两种角色，admin和user
菜单管理sys_menu:用户->角色->菜单->请求路径|权限
部门管理sys_dept:用户和部门是多对多，因为有的高层领导会负责多个部门。同时需要控制，有些数据只能本部门的人才能查看，其它部门不能查看
岗位管理sys_post:用户和岗位是多对多
 */;
...
) ENGINE=InnoDB AUTO_INCREMENT=21 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='字典表';
...
CREATE TABLE IF NOT EXISTS `sys_user` (
  ...
  `account_status` char(1) DEFAULT '0' COMMENT '帐号状态（0正常 1停用）',
  `remark` varchar(500) DEFAULT NULL COMMENT '备注',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
