...
2. 运行 `docker run --name spring-boot-docker -d -p 8080:8080 mondari/spring-boot-docker ` 命令运行容器
...
2. 使用 `docker run --name CONTAINRE_NAME -d -e "SPRING_PROFILES_ACTIVE=prod" -p HOST_PORT:CONTAINER_PORT -t IMAGE` 命令可以指定启动时使用的配置文件，`-d` 选项指定以守护进程的方式启动，`-t` 选项分配一个TTY终端
